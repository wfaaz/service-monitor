...
public class CallerScanner {
    ...
    private void parceLine(String line) {
        try {
            ...
            Caller caller = new ServiceStatusCaller(new InetSocketAddress(ip, port), pollTimeMs, graceTimeMs);
    ...
    private void parceGraceTime(String[] addressPollAndGraceParams) throws ServiceDataValidatorException {
        graceTimeMs = TimeUnit.SECONDS.toMillis(Long.parseLong(addressPollAndGraceParams[2]));
        throwWrapper(isValidTime(graceTimeMs));
    }

    private void parcePollTime(String[] addressPollAndGraceParams) throws ServiceDataValidatorException {
        pollTimeMs = TimeUnit.SECONDS.toMillis(Long.parseLong(addressPollAndGraceParams[1]));
        throwWrapper(isValidTime(pollTimeMs));
    }
